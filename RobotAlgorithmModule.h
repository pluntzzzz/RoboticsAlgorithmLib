...
typedef struct {
    ...
    double p2[3];   ////末端位置
    double L;       ////模长
    double t[3];    ////各方向基模长归一化
    double pi[3];   ////当前位置
    double Li;      ////当前位置到初始位置的模长
    int InpFlag;    ////初始化flag
} LineInpParam;
...
typedef struct {
    ...
    int InpFlag;    //1:finish initial 完成初始化,2;interpolating 插补中 ,3;finish interpolation 完成插补
} OrientInpParam;
...
*/
////位置插补和旋转插补的混合结构体，需要传入之前的LineInpParam，OrientInpParam，两个结构体
typedef struct {
    LineInpParam Line;
...
笛卡尔坐标变换（MDH表示）
*/
void DescartesTrans(MDHParam7 *MP, double T01[4][4],double T02[4][4],double T03[4][4],double T04[4][4],double T05[4][4],double T06[4][4],double T07[4][4]);


/**
空间雅克比
*/
void JacobSpace7(double T01[4][4], double T02[4][4], double T03[4][4], double T04[4][4], double T05[4][4],
